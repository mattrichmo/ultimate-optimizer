...
def insert_into_database(db_path: Path, image_data: Dict):
    ...
    """, (
        ...
        image_data.get("optimized_path"),
        image_data.get("resized_path"),
...
def optimize_image(image_path: Path, base_dir: Path, json_data: List[Dict], db_path: Path):
    """
    ...
    Args:
        ...
        json_data (List[Dict]): The list to accumulate JSON data.
        db_path (Path): The path to the SQLite database file.
    """
    ...
    # Calculate absolute paths relative to the base directory
    try:
        # Compute relative paths
        relative_original_path = new_original_path.relative_to(base_dir)
        relative_optimized_path = optimized_path.relative_to(base_dir)
        relative_resized_path = resized_path.relative_to(base_dir)

        # Format as /base_dir_name/subfolder/file.ext
        location_original = f"/{base_dir.name}/" + str(relative_original_path).replace('\\', '/')
        location_optimized = f"/{base_dir.name}/" + str(relative_optimized_path).replace('\\', '/')
        location_resized = f"/{base_dir.name}/" + str(relative_resized_path).replace('\\', '/')
    except ValueError:
        # If image is not under base_dir, use absolute paths
        location_original = str(new_original_path.resolve()).replace('\\', '/')
        location_optimized = str(optimized_path.resolve()).replace('\\', '/')
        location_resized = str(resized_path.resolve()).replace('\\', '/')
    ...
    image_record = {
        ...
        "original_path": location_original,
        "optimized_path": location_optimized,
        "resized_path": location_resized,
        "original_size_kb": original_details.get("file_size_kb"),
        "optimized_size_kb": optimized_details.get("file_size_kb"),
        "resized_size_kb": resized_details.get("file_size_kb"),
        "original_width": original_details.get("width"),
        "original_height": original_details.get("height"),
        "optimized_width": optimized_details.get("width"),
        "optimized_height": optimized_details.get("height"),
        "resized_width": resized_details.get("width"),
        "resized_height": resized_details.get("height")
    }

    # Append to JSON data list
    json_data.append(image_record)

    # Insert into SQLite database
    insert_into_database(db_path, image_record)

def process_directory(directory: Path, base_dir: Path, json_data: List[Dict], db_path: Path):
    """
    ...
    for root, dirs, files in os.walk(directory):
        for file in files:
            ...
            if file.lower().endswith(('.jpg', '.jpeg', '.png')):
                ...
                optimize_image(image_path, base_dir, json_data, db_path)

def save_json(json_path: Path, data: List[Dict]):
    """
    Saves the collected image data to a JSON file.

    Args:
        json_path (Path): The path to the JSON file.
        data (List[Dict]): The list of image records.
    """
    try:
...
def initialize_json(json_path: Path):
    """
...
def main():
    ...
    json_path = input_dir_path / "images.json"
    ...
    # Process directory
    process_directory(input_dir_path, input_dir_path, json_data, db_path)
